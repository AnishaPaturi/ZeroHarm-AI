    ...
    def generate_summary(self, plant_state: Dict[str, Dict[str, Any]], active_alerts: List[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if self.safety_agent and getattr(self.safety_agent, "openrouter_api_key", None):
            summary_desc = (
                ...
                f"- Active Gas Alerts: {'; '.join(gas_alerts) if gas_alerts else 'None'}\n"
                f"- Active Permits: {len(active_permits_summary)}\n"
                f"- High Risk Sectors: {', '.join([h['zone'] for h in high_risk_zones]) if high_risk_zones else 'None'}\n"
            )
            try:
                rag_res = self.safety_agent.query(
                    f"Create a formal, structured shift handover report based on this plant status: {summary_desc} "
                    f"under Factories Act Sec 87 compliance guidelines."
                )
                narrative = rag_res.get("answer", "")
            except Exception as e:
                logger.error(f"Error compiling RAG handover report: {e}")
